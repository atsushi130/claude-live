...
既定で `~/.local/share/claude-live-pet/pets/<名前>/`、`--out` で変えられる。
...
| 書き込み | `~/.local/share/claude-live-pet/` | 選んだペット・表示位置・**登録簿（`pets.json`）** | 手で消したときだけ |
| 登録したペットの実体 | 利用者が決めた任意の場所 | コマ画像 | 利用者が消したときだけ |
| 入口 | `~/.claude/plugins/data/claude-live-pet-<マーケットプレイス>/current` | いま入っている版への symlink | アンインストールで消える（毎セッション作り直す） |
...
**書き込み先をプラグインの外に置いているのは、アンインストールで消えないようにするため。**
プラグインごとの置き場所（`~/.claude/plugins/data/…`）は更新はまたげるが、
アンインストールでは消える（実測）。登録したペットは利用者が用意した素材で、
プラグインを一度外しただけで失われてよいものではない。消したいときは手で消す。
