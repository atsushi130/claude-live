...
`prepare` は**途中経過を流す**。導入は数分ブロックするので、何も出ないと止まっているのか
進んでいるのか分からない。MCP の進捗通知（`notifications/progress`）に日本語の 1 行を載せ、
「読み上げエンジンを確かめています」「ダウンロード中」「エンジンを起動しています」を
そのまま画面へ送る。**導入の手順はスクリプト側が持ったまま**で、ツールはその出力を中継するだけ。
