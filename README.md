...
| `speak` | 読み上げのみ |
| `listen` | 聞き取りのみ |
| `prepare` | エンジンと話者を揃えて起動する。最初に 1 回だけ呼ぶ |
| `finish` | 音声のやり取りを終えて片付ける。会話自体は続く |

### 声で流れる内容は画面にも出る

音声だけだと聞き逃したら終わりで、固有名詞や数値は目で追えた方が確かめやすい。
MCP の進捗通知（`notifications/progress`）に載せて、読み上げる文をそのまま画面へ送る。

```
🔊 認証まわりの設計を見直しました。要点は 3 つあります。
🎤 聞き取り中
```

`prepare` も同じ経路で途中経過を流す。導入は数分ブロックするので、何も出ないと
止まっているのか進んでいるのか分からない。**導入の手順はスクリプト側が持ったまま**で、
ツールはその出力を 1 行ずつ中継する。
...
音声のやり取りは呼び出し元と同じ会話で進む。**テキストで話していた途中から声に
切り替えられる**ようにするためで、それまでの文脈を引き継いだまま話し始められる。

引き換えに、往復の書き起こしがそのまま会話に積まれる。長く話したあとはコンテキストが
伸びるので、続きをテキストで進めるときは重くなる。

### 終わりはツールで知らせる

サブエージェントとして走らないので、終了を外から検知する仕組みが無い。
スキルは終わりに `finish` を呼び、音声に合わせて立ち上げたもの（画面のオーバーレイなど）を
片付ける。`finish` は音声のやり取りを閉じるだけで、会話はそのまま続く。

呼ばれないまま終わっても、次の 2 つは効く。

- **打ち切り**（`Esc` など）— MCP の打ち切り通知を受けて片付ける
- **セッション終了** — `SessionEnd` フックで片付ける

### 以前は別のコンテキストで走らせていた

`context: fork` を指定してサブエージェントとして動かしていた。往復の書き起こしが
メインに積まれない利点があり、終わりも `SubagentStop` で検知できた。

**やめたのは、テキストの続きから声に移れないため。** `context: fork` は呼び出し元の
会話を引き継がず（実測）、スキル本文を最初の入力として新しい会話を始める。
話していた内容を引き継ぐには、`/claude-live <話題>` の引数で毎回渡し直すしかなかった。
...
**沈黙している間はゼロ。** 待っている間は 1 件もリクエストが出ない（9 時間の実測）。
常駐させておいても、話しかけるまで消費はない。

**常に載っているのは 7 KB ほど。** スキル 5.4 KB とツール定義 1.7 KB だけで、
使っていないときの負担はこれで全部。

**話し始めると、1 往復あたり出力 500 トークンほど。** 入力は毎回ほぼキャッシュから
読まれる（15 往復のセッションで、新しく読ませた分は 119 トークン）。

| | 実測 |
| --- | --- |
| 沈黙中 | 0 |
| 常時のコンテキスト | 7 KB（スキル 5.4 KB + ツール定義 1.7 KB） |
| 1 往復の出力 | 約 490 トークン |
| 入力がキャッシュから読まれた割合 | 97.4% |

**弱点は往復数の多さ。** 音声は短いやり取りを重ねるので、テキストで 1 回に長く
答えてもらう場合より往復が増える。往復のたびに会話全体が読み直されるため、
**コンテキストが大きいほど 1 往復が高くつく**。

### 沈黙が長引いても増えない

費用が生じるのは聞き取りが**返ってきた瞬間**だけで、しかも安くない。
キャッシュには有効期間があり（実測時は 5 分）、それを超えて待つと会話全体を
書き込み直すことになる。書き込みは入力単価の 1.25 倍、読み出しは 0.1 倍で、
**12.5 倍の差**がある。

そのため**聞き直す回数を最小にする**設計にしている。1 回の呼び出しで 1 時間待ち、
発話が無ければそこで終了するので、沈黙のあいだに生じるのは
**終了を判断する 1 回だけ**になる。

待ち時間はツールの引数にしていない。短く刻むと有効期間の境界を何度も跨ぐため、
...
- **新しいセッションで始める**（`claude-live/skills/claude-live/scripts/start-claude-live` はそうしている）。
  読み直す量はコンテキストの大きさに比例するので、長い作業の続きで音声を始めると往復ごとに高くつく
